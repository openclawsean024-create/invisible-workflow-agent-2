...
export default function WorkflowsPage() {
  ...
  const handleSave = async () => {
    if (!ruleName.trim()) { setError('請輸入規則名稱'); return; }
    setSaving(true); setError(''); setSaved(false);
    try {
      const res = await fetch('/api/rules', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: ruleName, trigger, condition: '{}', action, enabled: true }),
      });
      if (!res.ok) throw new Error('儲存失敗');
      setSaved(true); setRuleName('');
    } catch (err: unknown) {
    ...
    <div className="min-h-screen bg-gray-950 text-gray-100">
      ...
      <div className="pt-24 pb-16 px-6">
        <div className="max-w-2xl mx-auto">
          <h1 className="text-2xl font-bold mb-1">工作流規則</h1>
          <p className="text-gray-400 text-sm mb-8">建立自動化工作流程</p>
          ...
          <div className="card">
            <h2 className="font-semibold mb-4">建立新規則</h2>
          ...
          <div className="mt-8 card">
            <h2 className="font-semibold mb-4">現有規則</h2>
            <p className="text-gray-500 text-sm">目前沒有已建立的規則</p>
          </div>
        </div>
      </div>
    </div>
